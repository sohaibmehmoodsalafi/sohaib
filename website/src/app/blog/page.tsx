      ...
      <main style={{ background: "var(--bg)", paddingTop: 96 }}>
        <section className="section-pad" style={{ padding: "60px 4vw 100px" }}>
          <div style={{ maxWidth: 1000, margin: "0 auto" }}>
            ...
            <h1 style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 800, fontSize: "clamp(36px,5vw,60px)", lineHeight: 1.02, letterSpacing: "-.02em", color: "var(--text)", marginBottom: 16 }}>
              Growth guides for <span style={{ color: "#d4a017" }}>academies &amp; Islamic organizations.</span>
            </h1>
            <p style={{ fontSize: 16, color: "var(--text-muted2)", maxWidth: 560, lineHeight: 1.7, marginBottom: 56 }}>
            ...
            <div className="blog-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 22 }}>
              {posts.map((p) => (
                <a key={p.slug} href={`/blog/${p.slug}`} className="blog-card" style={{ display: "flex", flexDirection: "column", background: "var(--bg-card)", border: "1px solid var(--hairline)", borderRadius: 18, padding: "30px 28px", textDecoration: "none", color: "var(--text)", transition: "border-color .2s, transform .2s" }}>
                  ...
                  <p style={{ fontSize: 14, color: "var(--text-muted2)", lineHeight: 1.7, marginBottom: 20, flex: 1 }}>{p.excerpt}</p>
