...
function TestiCard({ init, name, role, text, highlight }: { init: string; name: string; role: string; text: string; highlight: string }) {
  ...
  return (
    <div style={{
      background: "var(--bg-card)", border: `1px solid ${hov ? "var(--hairline-2)" : "var(--hairline)"}`,
      ...
      <p style={{ fontSize: 15, color: "var(--text)", lineHeight: 1.75, marginBottom: 28, fontWeight: 300 }}>
  ...
  return (
    <section ref={ref as React.RefObject<HTMLElement>} className="section-pad" style={{ padding: "120px 4vw" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 16, color: "var(--text-muted2)", maxWidth: 500, lineHeight: 1.7, marginBottom: 64 }}>Real clients. Real results. Ask me for references to verify directly.</p>
        <div className="testi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
          ...
          <div style={{
            ...
            <p style={{ fontSize: 14, color: "var(--text-muted2)", lineHeight: 1.7 }}>47+ verified clients across Pakistan, UAE, UK and USA. References available on request.</p>
            <a href="https://wa.me/923048885206" target="_blank" rel="noopener noreferrer"
              style={{ background: "transparent", color: "var(--text)", fontWeight: 500, fontSize: 13, padding: "10px 22px", borderRadius: 99, border: "1px solid var(--hairline-2)", textDecoration: "none", transition: "border-color .2s" }}
