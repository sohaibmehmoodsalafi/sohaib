...
function PortCard({ type, title, result, url, img, placeholder, icon, linkLabel }: Project) {
  ...
  return (
    <div style={{
      border: `1px solid ${hov ? "var(--hairline-2)" : "var(--hairline)"}`,
      borderRadius: 16, overflow: "hidden", background: "var(--bg-card)",
      transform: hov ? "translateY(-4px)" : "none",
      transition: "transform .25s, border-color .25s",
    }} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}>
      <div style={{ width: "100%", aspectRatio: "16/9", background: "var(--bg-card2)", overflow: "hidden", position: "relative" }}>
        {showPlaceholder ? (
          <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, background: "radial-gradient(circle at 50% 40%, rgba(212,160,23,.08), transparent 70%)" }}>
            ...
            <span style={{ fontSize: 15, fontWeight: 700, color: "var(--text)", padding: "0 20px", textAlign: "center", letterSpacing: ".01em" }}>{placeholder || title}</span>
          </div>
        ) : (
      ...
      <div style={{ padding: 24 }}>
        ...
        <p style={{ fontSize: 13, color: "var(--text-muted2)", display: "flex", alignItems: "flex-start", gap: 6 }}>
...
export default function Portfolio() {
  const ref = useFadeIn();
  return (
    <section ref={ref as React.RefObject<HTMLElement>} id="work" className="section-pad" style={{ padding: "120px 4vw", background: "var(--bg-dark)", borderTop: "1px solid var(--hairline)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: 16, color: "var(--text-muted2)", maxWidth: 520, lineHeight: 1.7, marginBottom: 64 }}>Quran academies, Islamic universities, and non-profits — every funnel, ad, and landing page handled by one person.</p>
        <div className="port-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
          ...
          <div style={{
            ...
            border: "1px solid var(--hairline)", borderRadius: 16, background: "var(--bg-card)",
          }}>
            <div style={{ fontSize: 40, marginBottom: 14 }}>📂</div>
            <p style={{ fontSize: 15, fontWeight: 700, marginBottom: 8 }}>More academies &amp; organizations</p>
            <p style={{ fontSize: 13, color: "var(--text-muted2)", marginBottom: 20 }}>Full case studies with enrollments, leads, and donation results.</p>
            <a href="https://wa.me/923048885206?text=Hi%20Sohaib!%20Please%20share%20your%20full%20client%20list%20for%20academies%20and%20Islamic%20organizations." target="_blank" rel="noopener noreferrer"
              style={{ background: "transparent", color: "var(--text)", fontWeight: 500, fontSize: 13, padding: "10px 22px", borderRadius: 99, border: "1px solid var(--hairline-2)", textDecoration: "none" }}
